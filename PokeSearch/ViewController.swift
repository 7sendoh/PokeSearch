...
//  PokeSearch
//
//  Created by Ping Li on 2017-02-08.
...
class ViewController: UIViewController, MKMapViewDelegate, CLLocationManagerDelegate {
    ...
    func mapView(_ mapView: MKMapView, annotationView view: MKAnnotationView, calloutAccessoryControlTapped control: UIControl) {
        if let anno = view.annotation as? PokeAnnotation{
            ...
            destination.name = "Pokemon Sightning"
    ...
    @IBAction func unwindFromPresentationVC(segue: UIStoryboardSegue) {
        if segue.identifier == "unwindFromPresentationVC" {
            ...
            {
                if presentationVC.selectedPokeId != -1
                {
                    let loc = CLLocation(latitude: mapView.centerCoordinate.latitude, longitude: mapView.centerCoordinate.longitude)
                    createSighting(forLocation: loc, withPokemon: presentationVC.selectedPokeId)
                }
            }
        }
    }
